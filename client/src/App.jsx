...
const API = import.meta.env.VITE_API_URL || 'https://skycalling.onrender.com';
const SIGNALING = import.meta.env.VITE_SIGNALING_URL || 'wss://skycalling.onrender.com';
...
function App() {
  ...
  async function callUser(targetId) {
    await startLocalStream({ video: false }); // только микрофон
  ...
  async function startLocalStream({ video = true } = {}) {
    ...
    if (video) localVid.srcObject = s;
    localStreamRef.current = s;
    if (!video) localVid.srcObject = null; // скрываем видео
  }

  async function acceptCall() {
    await startLocalStream({ video: false }); // сначала без видео
    const pc = createPeerConnection();
    pcRef.current = pc;
    localStreamRef.current.getTracks().forEach(t => pc.addTrack(t, localStreamRef.current));
  ...
  function toggleVideo() {
    if (localStreamRef.current) {
      ...
      const localVid = document.getElementById('localVideo');
      if (!videoOff) localVid.srcObject = null;
      else localVid.srcObject = localStreamRef.current;
